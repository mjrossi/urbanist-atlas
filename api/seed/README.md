...
| `regions_us_msas.toml`              | TOML            | 481 rows (393 MSAs + 88 portions) | Generated by `urbanist-atlas-server etl regenerate --country=US` from the Census CBSA delineation file. Override-aware. Cross-state MSAs emit a stateless umbrella + one `us:metro-portion` leaf per spanned state (see *Modeling conventions* below). **Do not hand-edit** — change `regions_us_msa_overrides.toml` and re-run instead. |
| `regions_us_msa_overrides.toml`     | TOML            | 8 rows (growing)              | Editorial overrides keyed by Census CBSA code. Pin friendly slugs (`nyc-metro`, `chicago-metro`, etc.) over the auto-generated `<first-city>-<state>-metro` defaults, and carry `rollup_states` for the two hand-curated flagships. |
...
- **Cross-state metros are stateless umbrellas.** A metro spanning multiple states carries no state edge (`parents = []`); each spanned state gets a `us:metro-portion` / `ca:cma-portion` leaf (`parents = [state, umbrella]`) that its ZIPs anchor at, and the umbrella carries a directional `rollup_states` list so the metro's own orgs still surface on each spanned state's `/region/{state}` page — browse direction only, never the ancestor walk, so no state leaks across the line. The two hand-curated flagships (`nyc-metro`, `chicago-metro`) instead route through curated borough/county leaves + the `nyc-tristate` / `chicagoland` advocacy nodes and currently skip portion generation. See [`docs/region-graph.md`](../../docs/region-graph.md) §1 for both shapes.
