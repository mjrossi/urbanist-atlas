...
| `regions_<cc>_cmas.toml`            | TOML            | 35 (CA), planned              | Generated by the etl subcommand from the StatsCan CMA reference (lands in slice #7.5.4). |
| `postal_codes_<cc>.csv`             | CSV (3 columns) | ~34k rows (US, generated)     | Generated by the etl subcommand from Census ZCTA crosswalks (US) and StatsCan PCCF (CA, planned). Smallest-anchor algorithm per the design spec. |
