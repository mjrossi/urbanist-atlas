...
//
// The two-query split (rather than one query with an optional filter)
// is forced by sqlc's SQLite parser, which rejects a named argument
// referenced twice — see the note in queries/usage_daily.sql.
