...
| D4 | **Counts buffer in memory and flush on an interval**, rather than writing per request. | SQLite then sees one small transaction per minute regardless of traffic spikes. This bounds — it does not eliminate — contention with the submission path: `sqlite.Open` sets `SetMaxOpenConns(1)`, so for the duration of one flush a concurrent `POST /submissions` or `/readyz` ping waits on the pool. The exposure is one batched transaction per flush interval rather than one per request, and the ceiling is small: ~1,550 keys/day at full catalog coverage is sub-millisecond, and even a buffer at the 10,000-key cap commits in ~0.2–0.3s, well inside Fly's probe timeout. Safe also because `fly.toml` already pins the app to one machine with `auto_stop_machines = false` — the same assumption the rate limiter and PR worker queue document. |
