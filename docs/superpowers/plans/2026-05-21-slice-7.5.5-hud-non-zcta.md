...
Run:
```bash
cd api && gofmt -l ./internal/etl/us/ && go vet ./internal/etl/us/... && mise exec -- staticcheck ./internal/etl/us/...
```
...
Run: `head -30 api/internal/store/postgres/pipeline_test.go`
...
Run: `cd api && go test -tags=integration -race -count=1 ./internal/store/postgres/ -run TestPipeline_HUDBackfill_ZIP20811 -v`
...
Run: `cd api && go test -tags=integration -race -count=1 ./internal/store/postgres/...`
Expected: all tests PASS.
...
Run: `grep -n "### Out-of-coverage UX" docs/superpowers/specs/2026-05-19-postal-coverage-design.md`
Expected: line ~295.
...
Run: `grep -n "7.5.5" docs/roadmap.md`
...
Run: `just api-test && just api-test-integration`
Expected: all tests PASS.
...
Run:
```bash
cd api && gofmt -l . && go vet ./... && mise exec -- staticcheck ./...
```
