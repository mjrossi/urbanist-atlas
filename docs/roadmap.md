...
- **Hosting cost spike (slice #19.5):** survey of May-2026 pricing
  ...
  (~$38/mo). Initial decision landed on Heroku ($12/mo total) on
  operator familiarity grounds and shipped as slice #20; **#20.6
  reversed that decision (2026-05-21) before any Heroku resources
  were provisioned**, back to the spike's original Finalist 1
  recommendation (Fly app + sibling `postgres:17-alpine`, ~$5/mo).
  See `docs/superpowers/specs/2026-05-18-hosting-cost-spike.md` for
  the comparison and
  `docs/superpowers/specs/2026-05-21-fly-redeploy-design.md` for
  the chosen design.
- **Heroku deploy deliverables (slice #20) — *retired by #20.6*:**
  shipped on 2026-05-19 as code/config/docs (Procfile,
  `URBANIST_DB_URL` → `DATABASE_URL` rename across every
  Postgres-touching urfave/cli flag, `loaddata` subcommand,
  `[group('heroku')]` recipes, Heroku runbook in `docs/deploy.md`)
  but never executed. Heroku-shaped artifacts (Procfile, heroku-*
  recipes, the original `docs/deploy.md`) are deleted by #20.6; the
  Go-side parts that were portability wins (`DATABASE_URL` rename,
  `loaddata` subcommand) survive unchanged. The branch reference is
  in git history.
...
|---|-------|------------|--------|
| 19 | **Dockerfile + fly.toml** *(retired by #19.5, restored by #20.6)* | Multi-stage Go build (binary embeds migrations); `fly launch`; `release_command = "urbanist-atlas-server migrate up"`. Shipped in slice #19, deleted by the #19.5 Heroku pivot, restored by #20.6 (Heroku → Fly re-pivot). The current `Dockerfile` + `fly.toml` reflect the slice-#20.6 design. | ✅ (then retired, then restored by #20.6) |
| **19.5** | **Hosting cost spike** | Verify Fly Managed Postgres pricing (~$38/mo Basic before storage), survey alternatives, pick a Phase 1 DB host that lands ≤ $5/mo all-in. Deliverable: `docs/superpowers/specs/2026-05-18-hosting-cost-spike.md`. Original decision (Heroku $12/mo) recorded in its Decision section was reversed by slice #20.6 (2026-05-21) back to the spike's Finalist 1 (Fly app + sibling `postgres:17-alpine`, ~$5/mo). | ✅ |
| 20 | **Heroku deploy + Postgres Essential-0** *(retired by #20.6)* | Heroku-shaped deliverables shipped on 2026-05-19 (`Procfile`, `URBANIST_DB_URL` → `DATABASE_URL` env rename, `loaddata` subcommand, `heroku-*` justfile recipes, Heroku runbook) but the runbook was never executed. Slice #20.6 reverses the Heroku decision and deletes the Heroku-shaped artifacts; the Go-side portability wins (`DATABASE_URL` rename, `loaddata` subcommand) survive unchanged. | ✅ (then retired) |
| **20.6** | **Fly redeploy + sibling Postgres + DNS + CORS + smoke** | Reverses the Heroku decision before any Heroku resources are provisioned (vendor-risk concerns + cost). **Deliverables (this slice):** `Dockerfile` + `fly.toml` at the repo root (multi-stage Alpine Go build, `release_command` runs migrations); `infra/postgres/fly.toml` for the sibling `urbanist-atlas-db` app running `postgres:17-alpine` on a 1 GB volume; `[group('fly')]` justfile recipes (`fly-deploy`, `fly-deploy-db`, `fly-logs`, `fly-logs-db`, `fly-secrets`, `fly-ssh`, `fly-loaddata`, `db-backup`, `db-restore`); `just smoke` recipe in `[group('smoke')]` covering slice #25 (curl checks against `/healthz`, `/api/v1/lookup` with + without `X-Atlas-Client`, ODbL headers, meta envelope, OpenAPI YAML); `.github/workflows/backup.yml` nightly cron + workflow_dispatch (pg_dump → R2, 30-day retention); `docs/deploy.md` rewritten end-to-end against Fly + sibling Postgres; `docs/superpowers/specs/2026-05-21-fly-redeploy-design.md` supersedes the Heroku design doc; `Procfile` deleted. Absorbs the DNS-retargeting half of #21, the CORS audit of #22, and the smoke-test code of #25. **Runbook execution (pending):** create both Fly apps (`flyctl launch` × 2 in region `iad`), create the 1 GB `pgdata` volume, set the four Fly secrets, `flyctl deploy`, seed via `just fly-loaddata`, create the Pages project, wire Cloudflare DNS (`qa` proxied → Pages, `qa-api` un-proxied → Fly), `flyctl certs add qa-api.urbanistatlas.com`, create R2 bucket with 30-day lifecycle, add the four GHA backup secrets, trigger `backup.yml` manually to verify. | ✅ deliverables / ▶ execute runbook |
| 21 | **Cloudflare Pages + DNS** | **Deliverables (shipped):** `web/public/_redirects` SPA fallback; the Pages project setup + DNS records documented in `docs/deploy.md` § Cloudflare Pages + DNS. **DNS retargeting absorbed by #20.6** (qa-api CNAME now points at `urbanist-atlas.fly.dev`, not a Heroku target). Pages env vars unchanged. **Runbook execution (pending — co-runs with #20.6):** create the Pages project (build cmd `cd web && npm ci && npm run build`, output `web/dist`, env vars `VITE_API_BASE` / `VITE_API_CLIENT_SECRET` / `NODE_VERSION=22` on Production + Preview), attach `qa.urbanistatlas.com` as Pages custom domain. | ✅ deliverables / ▶ execute runbook |
| 22 | **Production CORS (Phase 1 lockdown)** *(absorbed by #20.6)* | Audit that `URBANIST_CORS_ORIGINS` is locked to `https://qa.urbanistatlas.com` + `*.pages.dev` once dogfooding is live. No wildcard. Now baked into `fly.toml`'s `[env]` block; the audit is one of the `just smoke` checks in #20.6. (On prod cutover, swap in `https://urbanistatlas.com`.) | ✅ (absorbed) |
...
| 25 | **End-to-end smoke (Phase 1)** *(absorbed by #20.6)* | `just smoke` recipe hits the live QA endpoint: `/healthz` (200), `/api/v1/lookup?postal_code=10001&country=US` without secret (401), with secret (200 + `X-Data-License: ODbL-1.0` + `X-Data-Attribution` + `meta` envelope), `/api/v1/openapi.yaml` (200). Submissions + admin smoke deferred to Phase 2 with their slices. | ✅ (absorbed) |
