...
| 7.5.2 | **NYC borough split** | Migration `0004_split_nyc.sql` flips `nyc.scope_tier=regional`, drops the `nyc → ny` edge (boroughs carry the state edge per region-graph rule §1), keeps `nyc → nyc-metro`. Borough leaves already exist; only their parent edges shift. Editorial decision on whether to re-attach citywide NYC orgs (TransAlt, Riders Alliance, Brooklyn Spoke) to specific boroughs. |
| 7.5.3 | **US MSAs + ~33k ZCTA postal codes** | ETL parsers for Census CBSA + ZCTA-to-place + ZCTA-to-county. Generates `regions_us_msas.toml` (384 entries) + `regions_us_msa_overrides.toml` (top-50 hand-cleaned) + `postal_codes_us.csv` (~33k rows). Adds `UpsertPostalCodesBatch` (sqlc multi-row `unnest`) so loadpostal scales past per-row Heroku latency. Adds a mini-fixture seed dir for integration tests. |
| 7.5.4 | **CA CMAs + ~1.6k FSA postal codes** | ETL parsers for StatsCan PCCF (filtered to FSA/CSD/CMA/province columns) + StatsCan CMA reference. Generates `regions_ca_cmas.toml` (35 entries, all hand-cleaned) + `postal_codes_ca.csv` (~1.6k rows). Uses the batched loader from #7.5.3. Closes #7.5. |
