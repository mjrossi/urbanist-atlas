...
| 26 | **API key model — schema & issuance (Phase 2)** | Admin-issued `api_keys` (schema, sqlc store, issue/revoke endpoints + CLI, optional-key auth middleware) and self-serve email-verified registration. Detailed planning lives in the tracked issues: admin half [#118](https://github.com/mjrossi/urbanist-atlas/issues/118), public/email half [#120](https://github.com/mjrossi/urbanist-atlas/issues/120). Tracked under the Phase 2 umbrella [#36](https://github.com/mjrossi/urbanist-atlas/issues/36). | ⏳ |
| 27 | **Tiered rate limiting (Phase 2)** | Per-rate-key limiter (key when authed, IP for anonymous): tight anonymous budget, generous keyed budget, `429` + `Retry-After`. Detailed planning in [#119](https://github.com/mjrossi/urbanist-atlas/issues/119) (under [#36](https://github.com/mjrossi/urbanist-atlas/issues/36)). | ⏳ |
| 28 | **Phase 2 CORS narrowing + secret removal** | Apex DNS + cert + Workers custom-domain swap landed standalone 2026-05-27 (see Bring-up). Remaining: narrow CORS to `https://urbanistatlas.com`, remove the shared-secret middleware once #26–#27 are live, document keyed auth, key-tier telemetry dashboard. Detailed planning in [#121](https://github.com/mjrossi/urbanist-atlas/issues/121) (under [#36](https://github.com/mjrossi/urbanist-atlas/issues/36)). | ⏳ |
