...
> **Status: executed for Phase 1 QA (2026-05-21).** Both Fly apps
> (`urbanist-atlas`, `urbanist-atlas-db`), the Cloudflare Workers +
> Pages project (`urbanist-atlas`), `qa.urbanistatlas.com`, and
> `qa-api.urbanistatlas.com` are live behind the shared-secret gate.
> R2 backups workflow is still pending operator setup. The steps
> below remain the authoritative re-run sequence if the stack ever
> needs rebuilding from scratch.

The launch chunk lands as a single bring-up slice covering the
Fly + Cloudflare Workers + DNS + R2 backups + smoke + CORS work:

| Slice | What it ships | Where it's documented |
|---|---|---|
| Bring-up | Fly API + sibling Postgres + R2 backups + DNS + smoke + CORS audit | This file + the Fly deploy design doc |
| Web | Cloudflare Workers + Pages + DNS + TLS | **§ Cloudflare Workers + Pages + DNS** below |
| Gate | `X-Atlas-Client` shared-secret middleware | In tree; this runbook references the secret in § Secrets |
...
needs to be captured locally for the Cloudflare Workers + Pages
dashboard in § Cloudflare Workers + Pages + DNS, because Fly
secrets are write-only after set
...
echo "CLIENT_SECRET=$CLIENT_SECRET"   # → paste into password manager / Workers project dashboard NOW
...
`VITE_API_CLIENT_SECRET` value you set in the Cloudflare Workers +
...
The SPA deploys to Cloudflare Workers + Pages (Static Assets) — the
unified product Cloudflare migrated standalone Pages into. The
load-bearing config is **`wrangler.jsonc` at the repo root**, which
points the assets binding at `./web/dist` and enables SPA fallback
via `assets.not_found_handling = "single-page-application"`. The old
Pages `_redirects` catch-all (`/* /index.html 200`) is rejected by
Workers' stricter validator as a self-referential rewrite, so we
configure SPA fallback declaratively in `wrangler.jsonc` instead.

The order in this section matters: Fly issues the Let's Encrypt cert
for `qa-api.urbanistatlas.com` automatically, but only once DNS for
the hostname is resolvable — so the Fly side adds the cert request
first (§1), then DNS (§3), then the Workers + Pages side (§2/§4–§5).
...
`flyctl certs add` prints the **recommended A + AAAA records** —
something like `66.241.125.217` and `2a09:8280:1::119:4e6d:0`
(shared Fly anycast IPs). Capture those for §3.

### 2. Create the Workers project (dashboard)
...
- **Deploy command (Non-production):** `npx wrangler versions upload`
  *(default — keep it; promotes to a versioned preview URL without
  touching production traffic)*
- **Root directory:** repo root (leave blank — `wrangler.jsonc` is
  at the root)
- **Project name:** `urbanist-atlas` *(becomes
  `urbanist-atlas.<account>.workers.dev` for production and
  `<version>-urbanist-atlas.<account>.workers.dev` for previews)*

Build environment variables — **ensure they're in the *Build*
scope, not Worker runtime/secrets** (Vite inlines `VITE_*` at build
time; runtime bindings won't reach the bundled JS):
...
| `NODE_VERSION` | `22` *(major-version pin only; minor/patch may drift from `mise.toml`'s `22.22.3` pin, which is acceptable for a static SPA build)* |
...
The first push to `main` after this configures the project triggers
an automatic deploy. The first build runs `cd web && npm ci && npm run
build` then `npx wrangler deploy`, which reads `wrangler.jsonc`,
uploads `web/dist/` as static assets, and promotes the version to
production traffic.
...
|---|---|---|---|
| `qa-api` | A | *(IPv4 from §1, e.g. `66.241.125.217`)* | **OFF** *(Fly's edge terminates TLS directly)* |
| `qa-api` | AAAA | *(IPv6 from §1, e.g. `2a09:8280:1::119:4e6d:0`)* | **OFF** |

The `qa` SPA hostname is **NOT** a manual DNS record — Cloudflare
manages it automatically when you add the custom domain to the
Workers project in §4.

The `qa-api` proxy must be OFF because Fly is the TLS terminator. A
proxied `qa-api` would cause cert validation failures and break
Fly's automated Let's Encrypt issuance.

### 4. Custom domain for the SPA

In the Workers project: **Settings → Custom Domains → Add Custom
Domain → `qa.urbanistatlas.com`**. Cloudflare auto-creates the
DNS record on the zone (CNAME-flattening to the Workers anycast,
proxy on), provisions TLS via Universal SSL, and routes traffic to
the Workers project. Status badge flips to "Active" in 30s-2min.

### 5. Wait for the Fly cert

```sh
# Re-run until Status = Issued, "Certificate is verified and active".
flyctl certs show qa-api.urbanistatlas.com -a urbanist-atlas
```

Propagation is usually under 5 minutes once the `qa-api` A/AAAA
records in §3 go live; if it stalls beyond 15 minutes, verify (a)
the `qa-api` records resolve to the exact Fly anycast IPs
(`dig qa-api.urbanistatlas.com A +short` against `@1.1.1.1` to
bypass any local resolver cache), (b) the Cloudflare proxy on those
records is OFF, and (c) no DNSSEC or CAA record on the zone is
blocking Let's Encrypt issuance.
...
# All → 200, served by Workers (look for the cf-ray header).

# Confirm the SPA shell is served (not a Cloudflare default 404).
...
# index.html. (not_found_handling only fires for paths that DON'T
# match a real asset — real files short-circuit.)
...
# → must contain https://qa.urbanistatlas.com and *.<account>.workers.dev.

# API direct — no cf-ray header, Fly is the terminator.
curl -I https://qa-api.urbanistatlas.com/healthz
# → 200, no cf-ray.

# End-to-end smoke via the just recipe:
...
A throwaway branch push exercises the Workers preview path: `git
push origin throwaway-branch` triggers a non-production deploy via
`npx wrangler versions upload`, producing a versioned preview URL
like `https://<version>-urbanist-atlas.<account>.workers.dev`, which
uses the **Preview** env vars (so it still calls the QA API). The
`*.<account>.workers.dev` entry in `URBANIST_CORS_ORIGINS` keeps
these preview URLs working.
...
**`URBANIST_CLIENT_SECRET` ↔ `VITE_API_CLIENT_SECRET` (Workers project):**

```sh
NEW=$(openssl rand -hex 32)
flyctl secrets set URBANIST_CLIENT_SECRET="$NEW" -a urbanist-atlas
# Then in the Cloudflare Workers + Pages dashboard:
#   Workers & Pages → urbanist-atlas → Settings → Variables and Secrets
#   Build variables (Production + Preview): set VITE_API_CLIENT_SECRET = <NEW>
#   Trigger a redeploy (Deployments → Retry on latest, or push a commit)
echo "$NEW"   # paste into the Workers project dashboard
```

During the rotation window (Fly secret updated, Workers still
serving the old build): in-flight browser sessions will 401 until
the Workers project finishes rebuilding (~1 min). Schedule
rotations during low traffic.
...
1. **Cloudflare Workers + Pages**: in the `urbanist-atlas` project
   ...
   *second* custom domain. Cloudflare auto-creates the apex + `www`
   DNS records and provisions Universal SSL.
...
3. **CORS**: edit `fly.toml`'s `[env].URBANIST_CORS_ORIGINS` to
   ...
   code-coupled).
4. **Workers build env**: switch `VITE_API_BASE` from
   `https://qa-api.urbanistatlas.com` to
   `https://api.urbanistatlas.com`. Trigger a new Workers deploy.
5. **Verify** the prod hostnames serve correctly, then retire the
   QA hostnames: remove `qa-api` A/AAAA records, drop the QA cert
   from Fly (`flyctl certs remove qa-api.urbanistatlas.com`), remove
   the QA custom domain from the Workers project (Cloudflare cleans
   up its managed `qa` DNS record automatically), drop the
   `https://qa.urbanistatlas.com` entry from
   `URBANIST_CORS_ORIGINS`.
6. **Going forward**: rely on Workers' automatic versioned-preview
   URLs (`<version>-urbanist-atlas.<account>.workers.dev`) for
   ephemeral web environments.

No code change is required for this transition — it's all config
and DNS. The Fly apps, the Workers project, the Postgres volume,
...
`*.<account>.workers.dev` covers every Workers versioned-preview
hostname for that account.
