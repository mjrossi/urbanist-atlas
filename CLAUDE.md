...
- **Standard library first.** Pull a dependency only when stdlib genuinely
  ...
  - `sqlc` — SQL-to-Go bindings for the SQLite submission/usage store,
    generated from `api/internal/store/sqlite/queries/*.sql`. A
    build-time binary pinned in `mise.toml`, not a module dependency;
    regenerated by `just api-gen` via the `//go:generate` directive in
    `api/internal/store/sqlite/generate.go`, and drift-gated by
    `just api-gen-check`.
...
- **Config:** all via urfave/cli flags with env-var fallbacks
  ...
  `URBANIST_SEED_DIR`, `URBANIST_CLIENT_SECRET`,
  `URBANIST_USAGE_FLUSH_INTERVAL`, `URBANIST_USAGE_KEEP_DAYS`, etc.).
  No `viper`.
